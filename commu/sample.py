from robottools_commu import RobotTools
import time
...
rt = RobotTools('192.168.10.102', 22222) # <- IPアドレスを変更する
input()
...
input()
servo_map = dict(L_SHOU_P=-90, R_SHOU_P=0, BODY_Y=30)
...
servo_map = dict(HEAD_R=0, HEAD_P=-5, HEAD_Y=0, BODY_Y=0, 
                 L_SHOU_P=-90, L_SHOU_R=0, R_SHOU_P=90, R_SHOU_R=0)
pose = dict(Msec=1000, ServoMap=servo_map)
rt.play_pose(pose)
input()
...
input()
nod_motion = [
    dict(Msec=250, ServoMap=dict(R_SHOU_P=105,HEAD_P=-15,R_SHOU_R=0, L_SHOU_R=-3, L_SHOU_P=-102)),
    dict(Msec=250, ServoMap=dict(R_SHOU_P=77, HEAD_P=20, R_SHOU_R=17,L_SHOU_R=-17,L_SHOU_P=-79 )),
    dict(Msec=250, ServoMap=dict(R_SHOU_P=92, HEAD_P=-5, R_SHOU_R=5, L_SHOU_R=-7, L_SHOU_P=-88 ))
]
